...
public:
  ...
  KOKKOS_INLINE_FUNCTION void operator()(initGauge_cold_s, const int &x,
                                         const int &y, const int &z,
                                         const int &t, const int &mu) const {
    Group U;
    U.set_identity();
    this->gauge_field.set_link(x, y, z, t, mu, U);
  }

  KOKKOS_INLINE_FUNCTION void operator()(initGauge_hot_s, const int &x,
                                         const int &y, const int &z,
                                         const int &t, const int &mu) const {
  ...
  void initGauge(const bool cold_start) {
    if (cold_start) {
      auto BulkPolicy =
          Kokkos::MDRangePolicy<initGauge_cold_s, Kokkos::Rank<5>>(
              {0, 0, 0, 0, 0},
              {gauge_field.get_max_dim(0), gauge_field.get_max_dim(1),
               gauge_field.get_max_dim(2), gauge_field.get_max_dim(3),
               gauge_field.get_Ndim()});
      Kokkos::parallel_for("initGauge_cold", BulkPolicy, *this);
    } else {
      auto BulkPolicy = Kokkos::MDRangePolicy<initGauge_hot_s, Kokkos::Rank<5>>(
          {0, 0, 0, 0, 0},
          {gauge_field.get_max_dim(0), gauge_field.get_max_dim(1),
           gauge_field.get_max_dim(2), gauge_field.get_max_dim(3),
           gauge_field.get_Ndim()});
  ...
  T sweep() {
    auto BulkPolicy_odd = Kokkos::MDRangePolicy<sweep_s<1>, Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0},
        {gauge_field.get_max_dim(0), gauge_field.get_max_dim(1),
         gauge_field.get_max_dim(2), (int)(gauge_field.get_max_dim(3) / 2),
         gauge_field.get_Ndim()});
    auto BulkPolicy_even = Kokkos::MDRangePolicy<sweep_s<0>, Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0},
        {gauge_field.get_max_dim(0), gauge_field.get_max_dim(1),
         gauge_field.get_max_dim(2), (int)(gauge_field.get_max_dim(3) / 2),
         gauge_field.get_Ndim()});
